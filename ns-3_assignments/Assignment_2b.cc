...
NS_LOG_COMPONENT_DEFINE ("Mesh Csma");
...
main (int argc, char *argv[])
{
  ...
  for(int i = 0; i < no_nodes; i++) {
    for(int j = i + 1; j < no_nodes; j++) {
           NetDeviceContainer device = csma.Install({nodes.Get(i), nodes.Get(j)}) ;
  ...
  UdpEchoClientHelper echoClient (interfaces[interfaces.size() - 2].GetAddress (1), 9);
